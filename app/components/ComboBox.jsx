    ...
    return (
        <div className="comboBox">
            <div className="relative wrapper">
                <div className="flex items-center search tab__opt transition duration-200 ease-in-out hover:bg-[#d6e2ff] flex-1 h-full w-full lg:w-1/3 px-3 py-1.5 leading-loose text-[#222] bg-white/0 hover:bg-white/0 border border-gray-300  rounded-xl ">
                    <input
                        type="text"
                    ...
                    />
                    <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5C92FF">
                ...
                {dropdownVisible && (
                    <ul className="absolute left-0 right-0 mt-1 bg-white border border-gray-200 rounded-b-lg shadow-lg">
                        {filteredOptions.map((option) => (
                            <li key={option} className="px-4 py-2 cursor-pointer hover:bg-[#5C92FF] hover:text-white" onClick={() => handleOptionClick(option)}>
                                {option}
                            </li>
                        ))}
                    </ul>
                ...
                <div className="mt-2 text-gray-800">
                    <ul className="flex flex-wrap mt-1">
                        {selectedOptions.map((selectedOption) => (
                            <li key={selectedOption} className="px-3 py-1 mb-2 mr-2 text-white bg-[#5C92FF] rounded-full cursor-pointer" onClick={() => removeSelectedOption(selectedOption)}>
                                {selectedOption}
                                <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 ml-1 -mr-1 text-white cursor-pointer" fill="none" viewBox="0 0 24 24" stroke="currentColor">
